class Repl < User
   ...
   attr_accessor :line_length, :persistence, :new_session, :new_session_persisted, :check
    ...
    def before_loop
      ...
      #@check = @user.check(persistence)   #/ En caso de pedir registro al inicio/
    end

    def in_loop
      @new_session_persisted.first_folder if @persistence
      loop do
        #break unless (check == "passed") || (persistence)  # aun no se hizo el checkeo  de usuario con persistencia  /En caso de pedir registro al inicio/
        ...
        if (input === 'exit') || (input === 'quit')            
          ...
          begin
            ...
            when /create_file ([a-zA-Z0-9_.])+ (('(.{0,})')||("(.{0,})"))$/      #COMMAND: create_file file1 'c1'
              ...
              @new_session.add_new_file(input, current_user) if (!persistence && (@check == "passed"))
              puts "You need to log in first." if (!persistence && (@check != "passed"))
            ...
            when /^create_folder ([a-zA-Z0-9_.]+$)$/                             #COMMAND: create_folder folder1
              @new_session.add_new_folder(input) if (!persistence && (@check == "passed"))
              puts "You need to log in first." if (!persistence && (@check != "passed"))
            ...
            when /^destroy ([a-zA-Z0-9_.]+$)$/                                   #COMMAND: destroy file1/folder1    
              @new_session.destroy_file_folder(input) if (!persistence && (@check == "passed"))
              puts "You need to log in first." if (!persistence && (@check != "passed"))
            ...
            when /^whoami$/
              @user.whoami
            when /create_user ([a-zA-Z0-9_.])+ ((.{1,}))$/
              @user.create_user_command(input)
            when /login ([a-zA-Z0-9_.])+ ((.{1,}))$/
              @check = @user.login_user_command(input)
            else
              eval(input)  
            end
